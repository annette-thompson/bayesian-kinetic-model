...
def plot_predictive_time_vs_observable(
    ...
    fig, ax = plt.subplots(figsize=(6.0, 4.0))
    pred_low, pred_med, pred_high = np.nanpercentile(posterior_samples, [2.5, 50.0, 97.5], axis=0)
    ...
    ax.plot(x_values, pred_med, linewidth=2.0, color=posterior_color, label="Posterior Median")
    ax.errorbar(
        x_values,
        observed_values,
        yerr=observed_sigma,
...
def plot_predictive_observable_vs_initial_concentration_table(
    ...
    ax.errorbar(
        pred_bar_positions,
        pred_med,
        yerr=[pred_med - pred_low, pred_high - pred_med],
        fmt="none",
    ...
    if table_rows:
        ...
        fig.text(0.5, 0.03, "Initial Conditions", ha="center", va="center", fontsize=11)
    else:
...
def plot_predictive(
    ...
    for dataset in experiment.datasets:
        ...
        for output_name, column_name in dataset.column_mapping:
            ...
            use_time_series_plot = dataset_type in {"timeseries", "replicate_timeseries"}

            if use_time_series_plot:
            ...
            elif dataset_type in {"endpoint", "rate"}:
