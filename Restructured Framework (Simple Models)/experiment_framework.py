...
class ExperimentBundle:
    ...
    observables: dict[str, ObservableDefinition]
...
def load_experiment_bundle(
    ...
    base_init_conds = solver_params.get("init_conds", {})
    base_y0 = np.array(
        [float(base_init_conds.get(species, 0.0)) for species in species_names],
        dtype=np.float64,
    )
    ...
    base_condition_index = register_condition(base_y0)
    ...
    for dataset_config in dataset_configs:
        observable_name = dataset_config.get("observable")
        ...
        preview_outputs = observable.evaluate(
        ...
        column_mapping = _normalize_column_mapping(dataset_config, observable)
        ...
        for output_name in observable.output_names:
            if output_name not in preview_outputs:
                raise ValueError(
                    f"Observable '{observable_name}' did not produce expected output '{output_name}'."
                )
        ...
        condition_indices_np = _build_condition_indices_for_dataset(
            ...
            default_condition_index=base_condition_index,
        )
        ...
        for output_name, column_name in column_mapping:
            ...
            observed_labels.append(f"{dataset_config.get('name', observable_name)}::{output_name}")

        dataset_bundle = DatasetBundle(
            name=dataset_config.get("name", observable_name),
...
def validate_experiment_config(
    ...
    for dataset_config in normalized_configs:
        ...
        if dataset_config["noise_model"] not in noise_models:
            raise ValueError(
                f"Dataset '{dataset_name}' uses unsupported noise_model '{dataset_config['noise_model']}'. "
                f"Supported models: {sorted(noise_models.keys())}."
            )
...
def compute_observation_prediction(
    ...
    for dataset in experiment.datasets:
        observable = experiment.observables[dataset.observable_name]
        ...
        for output_name, _ in dataset.column_mapping:
            prediction_parts.append(outputs[output_name])
...
def format_experiment_summary(experiment: ExperimentBundle) -> str:
    ...
    for dataset in experiment.datasets:
        mapped_columns = ", ".join(
            f"{output}->{column}" for output, column in dataset.column_mapping
        )
        if dataset.init_cond_columns:
            ...
            init_mapping = "(global init_conds)"
        lines.append(
            f"  - {dataset.name}: type='{dataset.dataset_type}', observable='{dataset.observable_name}', "
...
def _build_sigma_array(
    ...
    noise_model_name = dataset_config.get("noise_model", "relative_mean")
...
def _build_condition_indices_for_dataset(
    ...
    if not init_cond_columns and not init_cond_overrides:
        return np.full((n_points,), default_condition_index, dtype=np.int32)
...
def _normalize_column_mapping(
    ...
    if column_mapping is None:
        return tuple((output_name, output_name) for output_name in observable.output_names)

    missing_outputs = [
        output_name for output_name in observable.output_names if output_name not in column_mapping
    ]
    if missing_outputs:
        raise ValueError(
            f"Dataset '{dataset_config.get('name', observable.name)}' is missing column mappings for outputs {missing_outputs}."
        )

    return tuple((output_name, column_mapping[output_name]) for output_name in observable.output_names)
...
def _normalize_dataset_config(
    ...
    if "observable" not in config:
        raise KeyError(f"Dataset index {dataset_index} is missing required key 'observable'.")
    if "data_file" not in config:
        raise KeyError(f"Dataset '{config.get('observable', dataset_index)}' is missing required key 'data_file'.")

    config.setdefault("name", f"dataset_{dataset_index}_{config['observable']}")
...
def _noise_column(
    ...
    **_: Any,
    ...
    column_mapping = dict(dataset_config.get("sigma_column_mapping", {}))
    column_mapping.update(dict(noise_params.get("column_mapping", {})))

    fallback_column = noise_params.get("column")
    sigma_column = column_mapping.get(output_name, fallback_column)
    if not sigma_column:
        raise ValueError(
            f"Noise model 'column' requires sigma column mapping for output '{output_name}'."
        )
    if sigma_column not in frame.columns:
        raise ValueError(
            f"Noise model 'column' references missing column '{sigma_column}'."
        )
...
def _noise_groupwise(
    ...
    **_: Any,
    ...
    min_sigma = float(noise_params.get("min_sigma", 1.0e-12))
    sigma = np.zeros(values.shape, dtype=np.float64)
    ...
    for group_value in unique_groups:
        ...
        if statistic == "std":
            local_sigma = float(np.std(group_values, ddof=1)) if group_values.size > 1 else min_sigma
        elif statistic == "sem":
            local_sigma = (
                float(np.std(group_values, ddof=1) / np.sqrt(group_values.size))
                if group_values.size > 1
                else min_sigma
            )
        elif statistic == "mad":
            median = float(np.median(group_values))
            local_sigma = 1.4826 * float(np.median(np.abs(group_values - median)))
        else:
            raise ValueError(
                "Noise model 'groupwise' supports statistic values: 'std', 'sem', 'mad'."
            )
        sigma[mask] = max(local_sigma, min_sigma)
...
def _resolve_path(base_dir: Path, candidate: str | Path) -> Path:
    ...
    # Prefer paths relative to the config directory, but also support project-relative
    # paths when solver_params.json is saved in a subdirectory (for example Results/).
    for search_dir in (base_dir, *base_dir.parents):
        resolved = (search_dir / candidate_path).resolve()
        if resolved.exists():
            return resolved
