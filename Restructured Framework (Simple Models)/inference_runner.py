...
def _build_pymc_model(
    ...
    if missing_free_params:
        raise ValueError(
            "Configured free parameters are missing from reaction parameters: "
            f"{missing_free_params}. Available reaction parameters: {param_names}."
        )

    missing_nominal_values = [name for name in param_names if name not in param_values]
    if missing_nominal_values:
        raise ValueError(
            "Missing nominal parameter values for reaction parameters: "
