#!/bin/bash
# Push local Bayesian Kinetic Model project files to nate (local lab GPU
# workstation, reached via `ssh nate`).
# Syncs selected project items, or all default items if none are given.
#
# Usage: ./sync_to_nate.sh [--dry-run] [--delete-not-on-source] [FOLDER_OR_FILE ...]
set -euo pipefail
...
Examples:
    ./sync_to_nate.sh
    ./sync_to_nate.sh --dry-run
    ./sync_to_nate.sh --dry-run --delete-not-on-source
    ./sync_to_nate.sh --delete-not-on-source Utilities Data Reactions
    ./sync_to_nate.sh Utilities Data Reactions
    ./sync_to_nate.sh "Bayesian Inference"
EOF
}
...
rsync "${RSYNC_OPTS[@]}" ${DRY_RUN:+$DRY_RUN} ${DELETE_NOT_ON_SOURCE:+$DELETE_NOT_ON_SOURCE} \
    --rsync-path="mkdir -p '$NATE_PROJECT' && rsync" \
    --exclude='__pycache__' \
    --exclude='.ipynb_checkpoints' \
    --exclude='*.pyc' \
    --exclude='.DS_Store' \
    "${RSYNC_SOURCES[@]}" "$(remote_rsync_path "${NATE_PROJECT}/")"
